...
import { createAgent, providerStrategy, toolStrategy, tool, StructuredOutputParsingError } from 'langchain';
...
/**
 * SYSTEM prompt for the tool-calling answer arm: identical two-source rules to {@link
...
/** The input message list for the tool-calling arm — like {@link buildAnswerMessages} but WITHOUT the
 *  inline record (the model pulls it via `get_patient_record`): just prior turns, then the question. */
